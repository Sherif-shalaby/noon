...
class Edit extends Component
{
    ...
    public function addStockProduct($line, $index = null)
    {
        //    dd( $line->used_currency);
        $variant = Variation::find($line->variation_id);
        $new_item = [
            ...
            'unit' => null,
            'unit_sku' => !empty($variant) ? $variant->sku : $line->sku,
            ...
            'customer_prices' => [],
            'stores' => [],
            'used_currency' => $line->used_currency ?? null,
            'dollar_purchase_discount' => null,
            'dollar_purchase_discount_percent' => null,
            'purchase_after_discount' => $line->purchase_after_discount ?? null,
            'dollar_purchase_after_discount' =>  $line->dollar_purchase_after_discount ?? null,
            'bonus_quantity' => !empty($line['bonus_quantity']) ? ($line['bonus_quantity']) : null,
            'discount_percent' => !empty($line['discount_percent']) ? ($line['discount_percent']) : null,
            'discount' => !empty($line['discount']) ? ($line['discount']) : null,
            'cash_discount' => !empty($line['cash_discount']) ? ($line['cash_discount']) : null,
            'seasonal_discount' => !empty($line['seasonal_discount']) ? ($line['seasonal_discount']) : null,
            'annual_discount' => !empty($line['annual_discount']) ? ($line['annual_discount']) : null,
            'seasonal_discount_amount' => !empty($line['seasonal_discount_amount']) ? ($line['seasonal_discount_amount']) : null,
            'annual_discount_amount' => !empty($line['annual_discount_amount']) ? ($line['annual_discount_amount']) : null,
            'purchase_discount' => !empty($line['purchase_discount']) ? ($line['purchase_discount']) : null,
            'invoice_discount' => !empty($line['cash_discount']) ? ($line['invoice_discount']) : null,
            // 'discount_on_bonus_quantity' => !empty($item['discount_on_bonus_quantity']) ? 1 : 0,
            'discount_dependency' => !empty($line['discount_dependency']) ? 1 : 0,
            'discount_on_bonus_quantity' => !empty($line['discount_on_bonus_quantity']) ? ($line['discount_on_bonus_quantity']) : null,
        ...
        if (!empty($product_prices)) {
            foreach ($product_prices as $price) {
                ...
                $customer_price = [];
                // dd($price);
                $customer_prices = VariationPrice::where('variation_id', $line->variation_id)->where('customer_type_id', $price['price_customer_types'])->get();
                // dd($customer_prices);
                $customer_price = [
                ...
                if (!empty($customer_prices)) {
                    foreach ($customer_prices as $c_price) {
                        $customer_name = CustomerType::where('id', $c_price->customer_type_id)->get('name');
                        $customer_price = [
                            'customer_type_id' => $c_price->customer_type_id,
                            'dinar_sell_price' => $c_price->dinar_sell_price,
                            'dollar_sell_price' => $c_price->dollar_sell_price,
                            'percent' => $c_price->percent,
                            'dollar_increase' => $c_price->dollar_increase,
                            'dinar_increase' => $c_price->dinar_increase,
                            'customer_name' => $customer_name,
                        ];
                    }
                }
                array_unshift($new_item['customer_prices'], $customer_price);
            }
        } else {
            $new_price = [
                'price_type' => null,
        ...
        $show_product_data = true;
        // $this->addNewProduct($variations, $product, $show_product_data,$index ,$line);
        if (!empty($new_item['stores'])) {
            foreach ($new_item['stores'] as $key => $item_store) {
                $this->addStockData($item_store, $line->transaction_id, $line->used_currency, $index, $key);
            }
        }
        // $customer_prices = $this->addCustomersPrice();
        array_unshift($new_item['prices'], $new_price);
        // dd($new_item);
        //    array_unshift($product['prices'],$new_price);
    ...
    public function store(): Redirector|Application|RedirectResponse
    {
        ...
        try {
            ...
            $transaction->store_id = $transaction->store_id ?? null;
            $transaction->status = 'received';
            $transaction->order_date = !empty($transaction->order_date) ? $transaction->order_date : Carbon::now();
            ...
            foreach ($this->items as $index => $item) {
                ...
                $add_stock_data = [
                    ...
                    'invoice_discount' => !empty($item['cash_discount']) ? ($item['invoice_discount']) : null,
                    //'bonus_quantity' =>!empty($item['bonus_quantity']) ? ($item['bonus_quantity']): null,
