...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromProduct($product_id, $variation_id = null, $store_id = null)
    {
        ...
        $products = $product->select(
            ...
            'product_stores.quantity_available',
        )
            ->get();
     ...
     * @param int $variation_id
     * @param int $store_id
     * @param bool $check_qty (If false quantity_available is not checked)
     *
     * @return object
     */
    ...
    /**
     * create or update remove add-stock lines
     *
    ...
    /*-- public function createOrUpdateAddStockLines($add_stocks, $transaction)
    {
        ...
        foreach ($add_stocks as $line) {
            if (!empty($line['add_stock_line_id'])) {
                $create.blade.php = AddStockLine::find($line['add_stock_line_id']);
                $create.blade.php->product_id = $line['product_id'];
                $create.blade.php->variation_id = $line['variation_id'];
                $old_qty = $create.blade.php->quantity;
                $create.blade.php->quantity = $line['bounce_qty'] > 0 ? $this->num_uf($line['quantity'])+$line['bounce_qty']: $this->num_uf($line['quantity']);
                $create.blade.php->purchase_price = $line['bounce_qty'] > 0 ? $line['bounce_purchase_price']:$this->num_uf($line['purchase_price']);
                $create.blade.php->final_cost = $this->num_uf($line['final_cost']);
                $create.blade.php->sub_total = $this->num_uf($line['sub_total']);
                $create.blade.php->batch_number = $line['batch_number'];
                $create.blade.php->manufacturing_date = !empty($line['manufacturing_date']) ? $this->uf_date($line['manufacturing_date']) : null;
                $create.blade.php->expiry_date = !empty($line['expiry_date']) ? $this->uf_date($line['expiry_date']) : null;
                $create.blade.php->expiry_warning = $line['expiry_warning'];
                $create.blade.php->convert_status_expire = $line['convert_status_expire'];
                $create.blade.php->sell_price = $line['selling_price'];
                $create.blade.php->bounce_qty = $line['bounce_qty'];
                $create.blade.php->profit_bounce = $line['bounce_profit'];
                $create.blade.php->bounce_purchase_price = $line['bounce_purchase_price'];
                $create.blade.php->bounce_convert_status_expire = $line['bounce_convert_status_expire'];
                $create.blade.php->bounce_expiry_warning = $line['bounce_expiry_warning'];
                $create.blade.php->bounce_expiry_date = $line['bounce_expiry_date'];
                $create.blade.php->bounce_manufacturing_date = $line['bounce_manufacturing_date'];
                $create.blade.php->bounce_batch_number = $line['bounce_batch_number'];
                $create.blade.php->save();
                ...
                $create.blade.php = AddStockLine::create($add_stock_data);
                $qty =  $this->num_uf($line['quantity']);
                if($create.blade.php){
                ...
                $keep_lines_ids[] = $create.blade.php->id;
                $batch_numbers[]=$create.blade.php->batch_number;
                $qty =  $this->num_uf($line['quantity']);
    ...
    /**
     * Checks if products has manage add-stock enabled then Decrease quantity for product and its variations
     *
     * @param $product_id
     * @param $variation_id
     * @param $store_id
    ...
    public function getProductDetailsUsingArrayIds($array, $store_ids = null,$variations= null)
    {
        ...
        $query->select(
            ...
            DB::raw('SUM(product_stores.quantity_available) as current_stock'),
    ...
    /**
     * get the add-stock value by store id
     *
     * @param int $store_id
     * @return void
     */
    public function getCurrentStockValueByStore($store_id = null)
    {
        if(!$store_id){
            ...
            foreach($stores as $store){
                $query = Product::leftjoin('variations', 'products.id', 'variations.product_id')
                    ->leftjoin('product_stores', 'variations.id', 'product_stores.variation_id')
                    ->leftjoin('add_stock_lines', 'variations.id', 'stock_lines.variation_id')
                    ->where('product_stores.store_id', $store->id)
                    ->where('add_stock_lines.purchase_price','>',0)
                ;
                $query->groupBy('variations.id')->select(
                    DB::raw('(product_stores.quantity_available ) * add_stock_lines.purchase_price as current_stock_value'),
                );
        ...
        $query->groupBy('variations.id', 'product_stores.quantity_available', 'add_stock_lines.purchase_price')
        ->select(
            DB::raw('(product_stores.quantity_available ) * add_stock_lines.purchase_price as current_stock_value')
        );
    ...
    public function getProductList($store_array = [], $sender_store_id = null)
    {
        ...
        if (empty($sender_store_id)) {
            ...
            $products = $products->select(
                ...
                'stores.id as store_id',
                // DB::raw('SUM(product_stores.quantity_available) as current_stock'),
                DB::raw('(SELECT SUM(product_stores.quantity_available) FROM product_stores WHERE product_stores.product_id=products.id AND product_stores.store_id=' . $sender_store_id . ') as current_stock')
            )->having('current_stock', '>', 0)
                ->groupBy('products.id', 'product_stores.id')
                ->get();
        }
    ...
    public function getCurrentStockDataByProduct($product_id, $store_id = null)
    {
        ...
        $current_Stock = $query->sum('quantity_available');
