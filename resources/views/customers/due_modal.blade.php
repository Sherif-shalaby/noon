                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control dueAmount" name="dueAmount"
                                value="{{ @num_format($dueAmount) }}" readonly>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="job_title">@lang('lang.due')</label>
                            <input type="text" class="form-control dueDollarAmount" name="dueDollarAmount"
                                value="{{ @num_format($dueDollarAmount) }}" readonly>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="due" id="due"
                                value="{{ @num_format($dueAmount) }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="job_title">@lang('lang.pay_dollar')</label>
                            <input type="text" class="form-control" name="due_dollar" id="due_dollar"
                                value="{{ @num_uf($dueDollarAmount) }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 add_to_customer_balance" hidden>
                        ...
                        <input type="hidden" name="add_to_customer_balance_dinar" id="add_to_customer_balance_dinnar"
                            class="add_to_customer_balance_in">
                        <input type="hidden" name="add_to_customer_balance_dollar" id="add_to_customer_balance_dollar"
                            class="add_to_customer_balance_in">
                    </div>
                </div>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#due_dollar').on('change', function() {
            ...
            if ($('#due_dollar').val() !== null && $('#due_dollar').val() !== 0) {
                console.log(1);
                if ($('.dueDollarAmount').val() == 0 && $('#due_dollar').val() !== 0 && $('#due')
                .val() !== 0 && $('.dueAmount').val() != 0) {
                    console.log(2);
                    $('.change_dollar').val($('.dueDollarAmount').val() - ($('#due_dollar').val() + ($(
                        '#due').val() / dollarExchange)));
                } else if ($('.dueDollarAmount').val() == 0 && $('.dueAmount').val() !== 0 && $(
                        '#due_dollar').val() !== 0 && $('#due').val() != 0) {
                    console.log(3);
                    $('.change_dinar').val($('.dueAmount').val() - ($('#due').val() + ($('#due_dollar')
                        .val() * dollarExchange)));
                } else if ($('.change_dinar').val() > 0 && $('.dueDollarAmount').val() !== null && $(
                        '.dueDollarAmount').val() !== 0 && $('#due_dollar').val() > $(
                        '.dueDollarAmount').val()) {
                    console.log(4);
                    var diff_dollar = $('#due_dollar').val() - $('.dueDollarAmount').val();
                    $('.change_dinar').val(round250($('.change_dinar').val() - (diff_dollar *
                        dollarExchange)));
                    $('.change_dollar').val(0);
                } else {
                    console.log(5);
                    if ($('.dueAmount').val() != 0 && $('.dueDollarAmount').val() == 0 && $('#due')
                        .val() == 0) {
                        console.log(6);
                        var rounded_final_total = round250($('.dueAmount').val());
                        $('.change_dinar').val(round250(rounded_final_total - ($('#due_dollar').val() *
                            dollarExchange)));
                        ...
                        if ($('.dueAmount').val() != 0) {
                            ...
                            if ($('.change_dinar').val() < 0 && $('.change_dollar').val() > 0) {
                                ...
                                $('.change_dollar').val($('.change_dollar').val() - (diff_dinar /
                                    dollarExchange));
                                $('.change_dinar').val(0);
                            }
                        }
        ...
        $('#due').on('change', function() {
            ...
            if (!isNaN(newAmount)) {
                ...
                if ($('#due').val() !== null && $('#due').val() !== 0) {
                    if ($('.dueAmount').val() == 0 && $('.dueDollarAmount').val() !== 0 && $(
                            '#due_dollar').val() !== 0 && $('#due').val() != 0) {
                        $('.change_dollar').val($('.dueDollarAmount').val() - ($('#due_dollar').val() +
                            ($('#due').val() / dollarExchange)));
                    } else if ($('.dueDollarAmount').val() == 0 && $('.dueAmount').val() !== 0 && $(
                            '#due_dollar').val() !== 0 && $('#due').val() != 0) {
                        $('.change_dinar').val($('.dueAmount').val() - ($('#due').val() + ($(
                            '#due_dollar').val() * dollarExchange)));
                    } else if ($('.change_dollar').val() > 0 && $('.dueAmount').val() !== null && $(
                            '.dueAmount').val() !== 0 && $('#due').val() > $('.dueAmount').val()) {
                        var diff_dinar = $('#due').val() - $('.dueAmount').val();
                        $('.change_dollar').val($('.change_dollar').val() - (diff_dinar /
                            dollarExchange));
                        $('.change_dinar').val(0);
                    } else {
                        if ($('.dueDollarAmount').val() != 0 && $('.dueAmount').val() == 0 && $(
                                '#due_dollar').val() == 0) {
                            $('.change_dollar').val($('.dueDollarAmount').val() - ($('#due').val() /
                                dollarExchange));
                        } else if ($('.dueAmount').val() != 0) {
                            $('.change_dinar').val(round_250($('.dueAmount').val()) - $('#due').val());

                            if ($('.dueDollarAmount').val() != 0) {
                                $('.change_dollar').val($('.dueDollarAmount').val() - $('#due_dollar')
                                    .val());
                                if ($('.change_dollar').val() < 0 && $('.change_dinar').val() > 0) {
                                    var diff_dollar = $('#due_dollar').val() - $('.dueDollarAmount')
                                        .val();
                                    $('.change_dinar').val(round_250($('.change_dinar').val() - (
                                        diff_dollar * dollarExchange)));
                                    $('.change_dollar').val(0);
                                }
                            }
                        }
