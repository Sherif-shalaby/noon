    ...
    <tr class="product_row">
        ...
        <td>
            @if (isset($product->quantity))
                {{ preg_match('/\.\d*[1-9]+/', (string) $product->quantity) ? $product->quantity : @num_format($product->quantity) }}@else{{ 1 }}
            @endif
        </td>
        <td style="width: 15%">
            <div class="input-group">
                <input type="text" class="form-control quantity" min=0
                    max="{{ preg_match('/\.\d*[1-9]+/', (string) $product->quantity) ? $product->quantity : @num_format($product->quantity) }}"
                    required wire:model="quantity.{{ $loop->index }}"
                    wire:change="changeAmount({{ $loop->index }})">
            </div>
