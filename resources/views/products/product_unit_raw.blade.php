    ...
    <input type="hidden" name="product[{{ $key ?? 0 }}]variation_ids[{{$index ?? 0 }}]" value="{{$variation->id ?? null}}">
    @if(isset($index) && $index!=='')
        @if($index == 1)
            ...
            <div class="col-md-2">
                ...
                <div class="d-flex justify-content-center">
                    <select name="products[{{ $key }}][variations][{{$index}}][new_unit_id]"  data-name='unit_id' data-index="{{$index}}" required class="form-control unit_select select2 unit_id{{$index}}" style="width: 100px;" data-key="{{ $key }}">
        ...
        @else
            <div class="col-md-2">
                ...
                {!! Form::text('products['.$key.'][variations]['.$index.'][equal]', isset($variation->equal)?$variation->equal:null, [
                    'class' => 'form-control'
                ]) !!}
            </div>
            <div class="col-md-2">
