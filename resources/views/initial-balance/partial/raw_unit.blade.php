        ...
        <div style="animation-delay: 1.65s"
         ...
         align-items-center p-2 rounded-3 text-center animate__animated animate__fadeInLeft @if (app()->isLocale('ar')) flex-row-reverse @else flex-row @endif ">
            ...
            <div style="
                ...
                <input type="text" style="width: 85%;font-size: 13px;font-weight:500;" class="mx-auto form-control sku"
            ...
            <div style="
                ...
                <div class="d-flex justify-content-center align-items-center" style="width: 65%;">
                    ...
                    <select wire:model="rows.{{ $index }}.basic_unit_id" data-name='basic_unit_id'
                        ...
                        <option value="">{{ __('lang.please_select') }}</option>
                        @foreach ($units as $unit)
                        <option value="{{ $unit->id }}" {{ isset($rows[$index]['basic_unit_id']) &&
                            $rows[$index]['basic_unit_id']==$unit->id ? 'selected' : '' }}>
                            {{ $unit->name }}</option>
                        @endforeach
                    </select>
                    <button type="button" class="btn btn-primary btn-sm ml-2 add_unit_raw" data-toggle="modal"
                ...
                <div class="d-flex justify-content-between align-items-center" style="width: 95%;">
                    ...
                    <span class=" dollar-cell showHideDollarCells">
                        ...
                        <select class="custom-select"
                            ...
                            wire:model="rows.{{ $index }}.fill_currency" wire:change="changeFilling({{ $index }})">
            ...
            <div style="
                     font-size: 12px;background-color: white;border-radius: 6px;margin: 6px;padding: 8px;height: 70px"
                class="dollar-cell showHideDollarCells  table-width px-0 d-flex justify-content-center align-items-center flex-column">
                <span style="font-weight: 700;
                ...
                <input type="text" class="form-control" wire:model="rows.{{ $index }}.dollar_purchase_price"
                    wire:change="changePurchasePrice({{ $index }})" style="width: 70px;font-size: 13px;font-weight:500;"
                    required>
            ...
            <div style="
                ...
                <input type="text" class="form-control " wire:model="rows.{{ $index }}.dollar_selling_price"
                    wire:change="changeSellingPrice({{ $index }})" style="width: 70px;font-size: 13px;font-weight:500;"
                    required>
            ...
            <div style="
                ...
                <span style="font-weight: 700;
                    ...
                    $</span>
                @if (isset($row['quantity']) && (!empty($row['dollar_purchase_price']) ||
                isset($row['purchase_price'])))
            ...
            <div style="
                ...
                @if (isset($rows[$index]['quantity']) && (isset($rows[$index]['purchase_price']) ||
                isset($dollar_purchase_price)))
                <span class="sub_total_span" style="font-size: 13px;font-weight:500;">
        ...
        <div class="accordion-item" style="border: none">
            <h2 class="accordion-header">
                ...
                <button class="accordion-button dis-button collapsed" style="padding: 5px 15px" type="button"
                    ...
                    data-index="{{ $index }}" aria-expanded="true" aria-controls="panelsStayOpen-collapse{{ $index }}"
            ...
            <div id="panelsStayOpen-collapse{{ $index }}"
                ...
                <div class="accordion-body mb-3 p-0 d-flex flex-wrap justify-content-between align-items-center py-2 rounded-3 text-center @if (app()->isLocale('ar')) flex-row-reverse @else flex-row @endif"
                    ...
                    <div class="d-flex flex-wrap  @if (app()->isLocale('ar')) flex-row-reverse @else flex-row @endif">
                        ...
                        <div
                            ...
                            <input type="text" class="form-control price_category " name="price_category"
                                wire:model="rows.{{ $index }}.prices.{{ $key }}.price_category" maxlength="6"
                        ...
                        <div
                            style="width: 100px;font-size: 12px;background-color: white;border-radius: 6px;margin: 6px;padding: 8px;">
                            {!! Form::label('b_qty', __('lang.b_qty'), ['style' => 'font-weight: 700;font-size: 11px',
                            'class' => 'pt-2']) !!}
                            ...
                            <div class="custom-control custom-switch">
                                <form id="myForm">
                                    <input type="checkbox" class="custom-control-input"
                                        ...
                                        style="font-size: 0.75rem" @if (isset($discount_from_original_price) &&
                                        $discount_from_original_price=='1' ) checked @endif
                                        wire:change="changePrice({{ $index }}, {{ $key }})">
                        ...
                        <div
                            ...
                            {!! Form::label(
                            'price',
                            isset($price['price_type']) && $price['price_type'] == 'fixed' ? __('lang.amount') :
                            __('lang.percent'),
                        ...
                        <div
                            style="width: 100px;font-size: 12px;background-color: white;border-radius: 6px;margin: 6px;padding: 8px;">
                            {!! Form::label('', __('lang.price'), ['style' => 'font-weight: 700;font-size: 11px',
                            'class' => 'pt-2']) !!}
                        ...
                        <div style="width: 100px;font-size: 12px;background-color: white;border-radius: 6px;margin: 6px;padding: 8px;"
                            class="dollar-cell showHideDollarCells">
                        ...
                        <div
                            ...
                            <div class="d-flex justify-content-center align-items-center" style=" border: none;
                                        border-radius: 16px;
                                        color: #373737;
                                        width: 100%;
                                        margin: auto;
                                ...
                                <select wire:model="rows.{{ $index }}.prices.{{ $key }}.price_customer_types"
                                    data-name='price_customer_types' data-index="{{ $index }}" data-key="{{ $key }}"
