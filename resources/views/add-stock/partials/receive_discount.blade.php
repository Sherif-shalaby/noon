<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editBrandModalLabel"
    ...
    <div class="modal-dialog" role="document" style="max-width: 600px!important;">
        <div class="modal-content">
            {!! Form::open([
                'url' => route('stocks.receive_discount', $transaction_id),
                'method' => 'post',
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 discount-div" id="seasonal_discount_divs">
                        @if ($seasonal_discount > 0)
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('seasonal_discount', $seasonal_discount, [
                                        'class' => 'form-control dueAmount',
                                        'placeholder' => __('lang.seasonal_discount'),
                        ...
                        @if ($seasonal_discount_dollar > 0)
                            <div class="col-md-6">
                                <div class="form-group">
                                    {!! Form::label('seasonal_discount', __('lang.seasonal_discount') . ':*', []) !!} <br>
                                    {!! Form::text('seasonal_discount_dollar', $seasonal_discount_dollar, [
                                        'class' => 'form-control dueDollarAmount',
                                        'placeholder' => __('lang.seasonal_discount'),
                                        'id' => 'seasonal_discount',
                                    ]) !!}
                    ...
                    <div class="col-md-6 discount-div" id="annual_discount_divs">
                        ...
                        @if ($annual_discount > 0)
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('annual_discount', $annual_discount, [
                                        'class' => 'form-control dueAmount',
                                        'placeholder' => __('lang.annual_discount'),
                                        'id' => 'annual_discount',
                                    ]) !!}
                        ...
                        @if ($annual_discount_dollar > 0)
                            <div class="col-md-6">
                                <div class="form-group">{!! Form::label('annual_discount', __('lang.annual_discount') . ':*', []) !!}
                                    <br>
                                    {!! Form::text('annual_discount_dollar', $annual_discount_dollar, [
                                        'class' => 'form-control dueDollarAmount',
                                        'placeholder' => __('lang.annual_discount'),
                                        'id' => 'annual_discount',
                                    ]) !!}
...
<script>
    ...
    $('#amount_dollar').on('change', function() {
        ...
        if ($('#amount_dollar').val() !== null && $('#amount_dollar').val() !== 0) {
            console.log(1);
            if ($('.dueDollarAmount').val() == 0 && $('#amount_dollar').val() !== 0 && $('#amount').val() !==
                0 && $('.dueAmount').val() != 0) {
                console.log(2);
                $('.change_dollar').val($('.dueDollarAmount').val() - ($('#amount_dollar').val() + ($('#amount')
                    .val() / dollarExchange)));
            } else if ($('.dueDollarAmount').val() == 0 && $('.dueAmount').val() !== 0 && $('#amount_dollar')
                .val() !== 0 && $('#amount').val() != 0) {
                console.log(3);
                $('.change_dinar').val($('.dueAmount').val() - ($('#amount').val() + ($('#amount_dollar')
                    .val() * dollarExchange)));
            } else if ($('.change_dinar').val() > 0 && $('.dueDollarAmount').val() !== null && $(
                    '.dueDollarAmount').val() !== 0 && $('#amount_dollar').val() > $('.dueDollarAmount')
                .val()) {
                console.log(4);
                ...
                if ($('.dueAmount').val() != 0 && $('.dueDollarAmount').val() == 0 && $('#amount').val() == 0) {
                    ...
                    $('.change_dinar').val(round250(rounded_final_total - ($('#amount_dollar').val() *
                        dollarExchange)));
    ...
    $('#amount').on('change', function() {
        ...
        if (!isNaN(newAmount)) {
            ...
            if ($('#amount').val() !== null && $('#amount').val() !== 0) {
                if ($('.dueAmount').val() == 0 && $('.dueDollarAmount').val() !== 0 && $('#amount_dollar')
                    .val() !== 0 && $('#amount').val() != 0) {
                    $('.change_dollar').val($('.dueDollarAmount').val() - ($('#amount_dollar').val() + ($(
                        '#amount').val() / dollarExchange)));
                } else if ($('.dueDollarAmount').val() == 0 && $('.dueAmount').val() !== 0 && $(
                        '#amount_dollar').val() !== 0 && $('#amount').val() != 0) {
                    $('.change_dinar').val($('.dueAmount').val() - ($('#amount').val() + ($('#amount_dollar')
                        .val() * dollarExchange)));
                } else if ($('.change_dollar').val() > 0 && $('.dueAmount').val() !== null && $('.dueAmount')
                    .val() !== 0 && $('#amount').val() > $('.dueAmount').val()) {
                    var diff_dinar = $('#amount').val() - $('.dueAmount').val();
                    $('.change_dollar').val($('.change_dollar').val() - (diff_dinar / dollarExchange));
                    $('.change_dinar').val(0);
                } else {
                    if ($('.dueDollarAmount').val() != 0 && $('.dueAmount').val() == 0 && $('#amount_dollar')
                        .val() == 0) {
                        $('.change_dollar').val($('.dueDollarAmount').val() - ($('#amount').val() /
                            dollarExchange));
                        ...
                        if ($('.dueDollarAmount').val() != 0) {
                            ...
                            if ($('.change_dollar').val() < 0 && $('.change_dinar').val() > 0) {
                                var diff_dollar = $('#amount_dollar').val() - $('.dueDollarAmount').val();
                                $('.change_dinar').val(round_250($('.change_dinar').val() - (diff_dollar *
                                    dollarExchange)));
                                $('.change_dollar').val(0);
                            }
                        }
